...
```
오디오/비디오 업로드 → 1분 자동 크롭 → STT (음성 → 텍스트) → 번역 → TTS (텍스트 → 음성) → 영상 합성 → 더빙 결과 비교 재생/다운로드
```
...
| 듀얼 미디어 뷰           | 원본과 더빙 결과를 나란히 비교 재생 (영상/오디오 자동 구분)  |
| 영상 더빙 합성            | FFmpeg.wasm으로 원본 영상 + 더빙 오디오를 합성한 영상 제공   |
| 클라이언트 미디어 크롭   | 1분 초과 파일을 클라이언트에서 자동 크롭 후 처리             |
...
| 기술         | 버전   | 용도                                        |
| ------------ | ------ | ------------------------------------------- |
| FFmpeg.wasm  | 0.12.x | 클라이언트 측 미디어 크롭 및 영상+오디오 합성 |
...
│   │   ├── ui/                   # DubbingForm, PipelineProgress, MediaPlayer 등
│   │   ├── model/                # useDubbingCreate hook (비즈니스 로직)
│   │   └── lib/                  # 파일 유효성 검사, 파이프라인 상태, 미디어 크롭 유틸
│   └── auth-login/               # 인증 피처
...
8개 기능 브랜치 전체에서 FSD 아키텍처가 일관되게 유지된 이유입니다.
...
| 듀얼 미디어 뷰              | `feat/#17-dual-media-view`    | spec.md → plan.md → tasks.md                 |
| 클라이언트 미디어 크롭      | `feat/#19-client-audio-crop`  | spec.md → plan.md → tasks.md                 |
